...
class EncryptTest extends GroovyTestCase {
    ...
    def sendCmd(int cmd, String payload) {
        int seqno = 1
        //state.seqno++
        //xxxx state.lastCommand = cmd
        sendLanCmd(seqno, cmd, payload)
    }
    ...
    byte[] packMessage(int seqno, int cmd, byte[] versionHeader, byte[] payload, byte[] localKey) {
        ...
        bufferLen += versionHeader.length + 3
        ...
        bufferLen = buffer.array().length
        bufferLen += Integer.BYTES // crc
        bufferLen += Integer.BYTES // suffix
        ByteBuffer buffer2 = ByteBuffer.allocate(bufferLen)
    ...
    Integer calculateCRC32(byte[] buffer) {
        CRC32 crc = new CRC32()
        crc.update(buffer)
