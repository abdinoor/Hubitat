...
@Field static final String APP_NAME = "Tuya LAN Integration"
@Field static final String NAMESPACE = "tuya"
@Field static final String VERSION = "1.0.0"

definition(
	name: "Tuya LAN Integration",
	namespace: NAMESPACE,
	author: "Dan Abdinoor",
	description: "Application to install Tuya devices for local-only control.",
	category: "Convenience",
	installOnOpen: true,
	singleInstance: true,
	importUrl: "https://raw.githubusercontent.com/abdinoor/Hubitat/refs/heads/master/Tuya-Local-App.groovy",
	iconUrl: "",
	iconX2Url: ""
)
...
def startPage() {
	logInfo("starting Tuya Integration")
	...
	try {
		...
		def segments = [hubIpArray[0],hubIpArray[1],hubIpArray[2]].join(".")
		app?.updateSetting("lanSegment", [type:"string", value: segments])
		app?.updateSetting("ports", [type:"string", value: "7000"])
		app?.updateSetting("hostLimits", [type:"string", value: "1, 254"])
	}
	return dynamicPage(name:"startPage",
		...
		section() {
			...
			href "lanAddDevicesPage",
				title: "<b>Scan LAN for Tuya devices and add</b>",
				description: "Primary Method to discover and add devices."

			paragraph " "
			href "removeDevicesPage",
				title: "<b>Remove Tuya Devices</b>",
				description: "Select to remove selected Tuya Device from Hubitat."
			paragraph " "

			input "utilities", "bool",
				title: "<b>Tuya Integration Utilities</b>",
				submitOnChange: true,
				defaultalue: false
			if (utilities == true) {
...
def addDevicesPage(discType) {
	...
	return dynamicPage(name:"addDevicesPage",
					   title: "Add Tuya Devices to Hubitat",
...
def listDevices() {
	...
	if (devices == null) {
		...
		theListTitle += "<b>Total Tuya Devices: ${devices.size() ?: 0}</b>\n"
	...
	return dynamicPage(name:"listDevices",
					   title: "List Tuya Devices from Add Devices",
					   nextPage: startPage,
					   install: false) {
	 	section() {
...
def findDevices() {
	...
	state.portArray.each {
		...
		state.segArray.each {
			...
			logInfo("findDevices: Searching for LAN devices on IP Segment = ${pollSegment}, port = ${port}")
...
def removeDevicesPage() {
	...
	return dynamicPage(name:"removedDevicesPage",
					   title:"<b>Remove Tuya Devices from Hubitat</b>",
					   nextPage: startPage,
					   install: false) {
...
def listDevicesByIp() {
	...
	deviceList.sort()
	def theListTitle = "<b>Total Tuya Devices: ${deviceList.size() ?: 0}</b>\n"
...
def listDevicesByName() {
	...
	return dynamicPage(name:"listDevicesByName",
					   title: "List Tuya Devices by Name",
					   nextPage: startPage,
					   install: false) {
	 	section() {
			paragraph theListTitle
	...
	return dynamicPage(name:"commsTest",
	 	...
	 	section() {
			def note = "This test measures ping from this Hub to any device on your  " +
				...
				"non-Tuya Device (other hub if you have one), and select the Kasa " +
			...
			input ("pingKasaDevices", "enum",
				   required: false,
				   multiple: true,
				   title: "Tuya Devices to ping (${kasaDevices.size() ?: 0} available).",
...
private xxxsendLanCmd(ip, port, cmdData, action, commsTo = 5) {
...
private sendLanCmd(ip, port, cmdData, action, commsTo = 5) {
	...
	String dummyPayload = 'deadbeef112233445566778899aabbccddeeffb00bface112233feedbabe74f0'
	// byte[] message = dummyPayload.getBytes("ISO-8859-1")
...
def asyncPassthrough(cmdBody, method, action) {
	if (devIp == null) { devIp = getDataValue("deviceIP") }	//	used for Tuya Compatibility
